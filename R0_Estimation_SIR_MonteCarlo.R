...
monte_carlo = function(data,iterations,range_beta,range_gamma,name,population,initialI,initialR,directory_name){
  ...
  for (iter in 1:iterations){
    ...
    if ((length(incidence_predicted)==length(incidence_observed))
      ...
      # last restriction is needed in order to work with the Poisson negative log likelihood (log0 is undefined)
...
for (country in c("Germany","India", "South Africa", "Peru", "Russia") ) { 
  ...
  #print(df %>% ggplot(aes(x = days_rel_start, y = removed)) + geom_point() + theme_bw() + scale_x_continuous("Days from start of the pandemic", breaks=c(0:50)*5) + theme(axis.text.x = element_text(angle = 90)))
  ...
  nrows_range = seq(peak-5, peak+5) # sample 11 intervals centered at peak estimate
